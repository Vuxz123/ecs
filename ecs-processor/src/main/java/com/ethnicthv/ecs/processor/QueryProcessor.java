...
""";
    ...
    private String buildRunParallelBlock() {
        return "  private void runParallel(Object managedSharedFilter, java.util.List<com.ethnicthv.ecs.core.archetype.ArchetypeQuery.UnmanagedFilter> unmanagedSharedFilters){ final var cm = world.getComponentManager(); final boolean hasFilter = (managedSharedFilter != null || (unmanagedSharedFilters != null && !unmanagedSharedFilters.isEmpty())); java.util.List<Runnable> tasks = new java.util.ArrayList<>(); for (com.ethnicthv.ecs.core.archetype.Archetype archetype : world.getAllArchetypes()) { com.ethnicthv.ecs.core.archetype.ComponentMask am = archetype.getMask(); if (!am.containsAll(withMask)) continue; if (!am.containsNone(withoutMask)) continue; if (anyIds.length > 0 && !am.intersects(anyMask)) continue; ArchetypePlan plan = PLAN_CACHE.computeIfAbsent(archetype, a -> buildPlan(a)); if (plan == null) continue; if (hasFilter) { com.ethnicthv.ecs.core.archetype.SharedValueKey qk = buildQueryKey(archetype, managedSharedFilter, unmanagedSharedFilters); if (qk == null) continue; com.ethnicthv.ecs.core.archetype.ChunkGroup g = archetype.getChunkGroup(qk); if (g == null) continue; com.ethnicthv.ecs.core.archetype.ArchetypeChunk[] chunks = g.getChunksSnapshot(); int count = g.chunkCount(); for (int i = 0; i < count; i++) { final com.ethnicthv.ecs.core.archetype.ArchetypeChunk c = chunks[i]; final ArchetypePlan p = plan; tasks.add(() -> runOnChunk_Parallel(c, p, cm)); } } else { for (com.ethnicthv.ecs.core.archetype.ChunkGroup g : archetype.getAllChunkGroups()) { com.ethnicthv.ecs.core.archetype.ArchetypeChunk[] chunks = g.getChunksSnapshot(); int count = g.chunkCount(); for (int i = 0; i < count; i++) { final com.ethnicthv.ecs.core.archetype.ArchetypeChunk c = chunks[i]; final ArchetypePlan p = plan; tasks.add(() -> runOnChunk_Parallel(c, p, cm)); } } } } tasks.parallelStream().forEach(Runnable::run); }\n";
    }
